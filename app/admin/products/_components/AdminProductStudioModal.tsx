...
import { AdminImageGalleryPicker } from './AdminImageGalleryPicker';
...
interface AdminProductStudioModalProps {
    isOpen: boolean;
    onClose: () => void;
...
interface InnerStudioFormProps {
    initialProduct?: Product | null;
    onClose: () => void;
    onSaveProduct: (productData: Omit<Product, 'id'> & { id?: string }) => void;
}
        ...
        <>
            ...
            <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between bg-slate-900/60 backdrop-blur-md">
                <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-2xl bg-amber-400/10 border border-amber-300/30 flex items-center justify-center text-amber-300 shadow-inner">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">
                            Configure sizes, visuals, pricing, and live streetwear styling with real-time preview.
                        </p>
                    </div>
                </div>
                <Button
                ...
                >
                    ...
                    <div className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/50 p-5">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="prod-name" className="text-xs text-slate-300 font-medium">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-[11px] text-slate-400">Selected: <strong className="text-amber-300">{effectiveCategory}</strong></span>
                            </div>
                    ...
                    <div className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/50 p-5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-xs text-slate-300">Compare-at Price (Sale)</Label>
                                <div className="relative">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-slate-400">R</span>
                                    <Input
                                        type="number"
                    ...
                    <div className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/50 p-5">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    type="button"
                                    onClick={() => setSizes(['XS', 'S', 'M', 'L', 'XL', 'XXL', '3XL'])}
                                    className="text-[11px] text-slate-400 hover:text-amber-300 underline"
                                >
                                    All (XS-3XL)
                                </button>
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            >
                                One Size
                                {sizes.includes('One Size') && <Check size={12} className="stroke-[3]" />}
                            </button>
                        </div>
                        ...
                        {sizes.some((s) => !STANDARD_SIZES.includes(s as (typeof STANDARD_SIZES)[number]) && s !== 'One Size') && (
                            <div className="flex flex-wrap gap-1.5 pt-1">
                                <span className="text-[11px] text-slate-400 self-center">Custom sizes:</span>
                                {sizes
                    ...
                    <div className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/50 p-5">
                            ...
                            <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-white/10">
                                <button
                                    ...
                                    className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all ${
                                        imageInputMode === 'upload' ? 'bg-amber-300 text-slate-950' : 'text-slate-400 hover:text-white'
                                    }`}
                                >
                                    Upload
                                </button>
                                <button
                                    ...
                                    className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all ${
                                        imageInputMode === 'gallery' ? 'bg-amber-300 text-slate-950' : 'text-slate-400 hover:text-white'
                                    }`}
                                >
                                    Gallery
                                </button>
                                <button
                                    ...
                                    className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all ${
                                        imageInputMode === 'url' ? 'bg-amber-300 text-slate-950' : 'text-slate-400 hover:text-white'
                                    }`}
                                >
                                    URL / Path
                                </button>
                        ...
                        {imageInputMode === 'upload' && (
                            ...
                            >
                                ...
                                <p className="text-xs text-slate-400 mt-1">
                                    Supports PNG, JPG, WEBP up to 5MB. Stored instantly with high visual fidelity.
                                </p>
                            </div>
                        )}
                        ...
                        {imageInputMode === 'gallery' && (
                            <div className="space-y-3">
                                ...
                                >
                                    <Sparkles className="h-4 w-4" /> Open Brand Photography Gallery
                                </Button>
                                <p className="text-xs text-slate-400 text-center">
                                    Browse official Drippy Banks catalog images from tees, caps, hoodies, and bags.
                                </p>
                            </div>
                        )}
                        ...
                        {imageInputMode === 'url' && (
                            <div className="space-y-2">
                                <Label className="text-xs text-slate-300">Public Path or Web URL</Label>
                                <Input
                                    value={image}
                        ...
                        {image && (
                            <div className="flex items-center gap-4 p-3 rounded-xl bg-slate-950 border border-white/10">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs font-semibold text-white truncate">Active Product Visual</p>
                                    <p className="text-[11px] text-slate-400 truncate">{image.startsWith('data:') ? 'Custom Uploaded Data' : image}</p>
                                </div>
                                <Button
                                    type="button"
                                    size="sm"
                    ...
                    <div className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/50 p-5">
                        <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-amber-300">
                            <SlidersHorizontal className="h-3.5 w-3.5" /> 5. Colorways & Garment Details
                        </div>

                        <div className="space-y-2">
                            <Label className="text-xs text-slate-300 font-medium">Available Colorways</Label>
                            <div className="flex flex-wrap gap-2">
                        ...
                        <div className="space-y-2 pt-1">
                            <Label className="text-xs text-slate-300 font-medium">Product Narrative & Fabric Notes</Label>
                            <Textarea
                                value={description}
                ...
                <div className="lg:col-span-5 bg-gradient-to-b from-slate-900/90 to-slate-950 p-6 flex flex-col justify-between overflow-y-auto max-h-[calc(92vh-130px)]">
                    <div className="space-y-4">
                        <div className="flex items-center justify-between pb-2 border-b border-white/10">
                            <span className="text-xs uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                                <Eye className="h-3.5 w-3.5 text-cyan-300" /> Live Storefront Preview
                            </span>
                    ...
                    <div className="mt-6 p-4 rounded-2xl bg-white/[0.03] border border-white/10 text-xs text-slate-400 space-y-1">
                        <p className="font-semibold text-slate-300 flex items-center gap-1.5">
                            <Sparkles className="h-3.5 w-3.5 text-amber-300" /> Admin Studio Tips
                        </p>
                        <p>
                            Changes take effect across the entire shop immediately upon publishing. You can also duplicate products in the catalog to quickly make color or silhouette variants.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-900/80 backdrop-blur-md">
                <div className="text-xs text-slate-400">
                    {isEdit ? 'Editing active catalog entry' : 'New product will be added directly to the catalog'}
                </div>
